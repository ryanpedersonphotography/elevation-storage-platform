        ...
        <Box className="max-w-7xl mx-auto px-6" py="7">
          <Grid columns={{ initial: '1', sm: '2', md: '4' }} gap="6">
            <Flex direction="column" gap="2">
              ...
              {branding.showParent && (
                <Text size="1" className="text-[var(--gray-9)]" mt="2">
            ...
            <Flex direction="column" gap="2">
              <Text size="2" weight="bold" className="text-white">Quick Links</Text>
              {enabledPages.map(([slug]) => {
                ...
                return (
                  <Link key={slug} href={href} className="no-underline">
                    <Text size="2" className="text-[var(--gray-8)]">{label}</Text>
                  </Link>
                )
              })}
            </Flex>
            ...
            <Flex direction="column" gap="2">
              <Text size="2" weight="bold" className="text-white">Storage Features</Text>
              {facility.data.amenities.slice(0, 5).map((a) => (
                <Text key={a.id} size="2" className="text-[var(--gray-8)]">{a.label}</Text>
              ))}
            </Flex>

            <Flex direction="column" gap="2">
              <Text size="2" weight="bold" className="text-white">Contact</Text>
              <a href={`tel:${info.phone}`} className="no-underline">
                <Text size="2" className="text-[var(--gray-8)]">{info.phone}</Text>
              </a>
              <a href={`mailto:${info.email}`} className="no-underline">
                <Text size="2" className="text-[var(--gray-8)]">{info.email}</Text>
              </a>
              <Box mt="2">
                <Text size="2" weight="bold" className="text-white">Hours</Text>
                {info.hours.map((h, i) => (
                  <Text key={i} size="1" className="text-[var(--gray-8)]" as="p">
                    {formatHoursLine(h)}
                  </Text>
                ))}
              </Box>
